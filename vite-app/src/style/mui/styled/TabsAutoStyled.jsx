...
export default function TabsAutoStyled({ originalTabs = [], defaultVal = 0, style = {}, searchVal = 'valueId', permissions = [], isPerms = false }) {
    ...
    const tabs = useMemo(() => {
        const modified = originalTabs.filter((tab) => {
            if (isPerms) {
                return tab.perm
            } else if (permissions.length && (tab.permId || tab.permId === 0)) {
                return permissions[tab.permId] ?? false
            } else {
                return true
            }
        })

        return modified.map((tab, i) => ({
            ...tab,
            value: tab.value ?? i
        }));
    }, [originalTabs]);
    ...
    const component = useMemo(() => tabs.find(t => t.value === safeValue)?.component, [value, tabs])
