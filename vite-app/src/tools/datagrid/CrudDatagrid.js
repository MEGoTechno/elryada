...
const BooleanSwitchCell = ({ field, row, updateFc, params }) => {
    const handleChange = async () => {
        const oldValue = row[field] ?? params.value
        const newRow = { [field]: !oldValue, _id: row._id, id: row._id }
        try {
            await updateFc(newRow, field);
            params.api.updateRows([{ ...newRow }])
        } catch (error) {
            console.log('error from CrudDataGrid BooleanSwitchCell ==>', error)
        }
    };

    return <SwitchStyled checked={row[field] ?? params.value} onChange={handleChange} disabled={!updateFc} />
};
...
function CrudDatagrid(
    ...
    const modifiedColumns = useMemo(() => {
        ...
        const switchedCols = columns.filter(col => !removeCols.includes(col.field)).map(col => {
