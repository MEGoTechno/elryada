...
function CreateCode({ setReset, lecture = false, chapter }) {
    ...
    const walletInputs = [
        {
            ...
            label: 'سعر الكود',
            type: 'number',
            validation: Yup.number().required(lang.REQUERIED).max(2000, "اقصى مبلغ هو 2000 ريال")
        }
    ]
    const centerInputs = [
        {
    ...
    const onSubmit = async (values, props) => {
        await createCode({ ...values, chapter })
        if (setReset) {
            setReset(pre => !pre)
        }
        props.resetForm()
