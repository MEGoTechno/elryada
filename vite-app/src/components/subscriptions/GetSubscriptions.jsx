...
import { useDeleteSubscriptionMutation, useLazyAnalysisSubscriptionsQuery, useLazyGetCourseSubscriptionsQuery, useUpdateSubscriptionMutation } from '../../toolkit/apis/userCoursesApi'
...
function GetSubscriptions({ courseId = '', user = '', isShowTitle = false }) {
    ...
    const removeFc = async (id) => {
        await deleteSubscription({ _id: id, course: courseId })
    }
    ...
    const updateFc = async (data) => {
        await updateSubscription({ id: data._id, course: courseId, ...data })
        return data
    }
    ...
    const columns = [
        ...
        },
        {
            ...
            field: 'progress',
            headerName: 'عرض التقدم',
            width: 150,
            type: 'actions',
            renderCell: (params) => {
                const chaptersProgress = params.row.chapters
                return (
                    <Box>
                        <BtnModal titleInSection={'تقدم الطالب : ' + params.row.name} btnName={"عرض التقدم"}>
                            <FlexColumn gap={'12px'}>
                                {chaptersProgress?.length && chaptersProgress.map(ch => {
                                    return <ModifyProgress updateFc={updateFc} setReset={setReset} key={ch._id} params={params} chapter={ch} />
                                })}
                            </FlexColumn>
                        </BtnModal>
                    </Box>
            ...
            width: 200,
            renderCell: (params) => {
                return <TabInfo count={(params.row.price) + ' ريال'} i={0} />
            }
        }, {
            field: 'createdAt',
    ...
    return (
        <Box sx={{ width: '100%' }}>
            ...
            <MeDatagrid
                ...
                // updateFc={updateFc} 
                loading={status.isLoading || updateStatus.isLoading || isLoading} deleteFc={removeFc}
                editing={
                    {
