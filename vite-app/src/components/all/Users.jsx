...
function Users({ filters, reset, viewFc, deleteFc, updateFc, massActions, allStatuses, addColumns, removeCols = [] }) {
        ...
        // },
         {
            ...
            renderCell: (params) => {
                const government = governments.filter(({ region_id }) => Number(region_id) === Number(params.row.government))[0]
                return (
                    <Typography>
                        {params.row.role === user_roles.ADMIN ? user_roles.ADMIN
                            : government?.name_ar}
