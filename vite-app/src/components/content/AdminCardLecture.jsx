...
function AdminCardLecture({ lecture, i, setLectures, courseId, chapter }) {
  const [open, setOpen] = useState(false)
  const isNativeLecture = (lecture?.course?._id === courseId || lecture?.course === courseId)

  const [hasPermission] = usePermissions()
  const canUpdate = hasPermission(chapterPerms(chapter._id, 'lecturesUpdate'))
  const canDelete = hasPermission(chapterPerms(chapter._id, 'lecturesDelete'))
  const canMakeCodes = hasPermission(chapterPerms(chapter._id, 'lecturesCodes', { courseId }))
  const showStatistics = hasPermission(chapterPerms(chapter._id, 'lecturesStatistics', { courseId }))
  ...
  const changeStatus = async (object) => {
    if (!canUpdate) {
      return alert('لا يمكنك تعديل المحاضره')
    }
    const res = await updateLecture({ id: lecture._id, ...object, chapters: lecture.chapters }, true)
  ...
  const triggerDelete = async () => {
    if (!canDelete) {
      return alert('لا يمكنك حذف المحاضره')
    }
    await deleteLecture({ id: lecture._id, chapters: lecture.chapters })
    setLectures(prev => {
      let lectures = [...prev]
          ...
          <>
            ...
            <FlexRow>
              ...
              <BtnModal
                ...
                ]} disabledBtn={!canUpdate} onSubmit={changeStatus} status={{ isLoading }} formDirection={'row'} btnStyle={{ width: 'fit-content' }} />}
              />
              <SwitchStyled disabled={!canUpdate} label={"قابله للبيع"} checked={lecture.isSalable} onChange={(value) => changeStatus({ isSalable: value })} isLoading={isLoading} />
            </FlexRow>

            <SwitchStyled disabled={!canUpdate} label={"الحاله"} checked={lecture.isActive} onChange={(value) => changeStatus({ isActive: value })} isLoading={isLoading} />
            <SwitchStyled disabled={!canUpdate} label={"جعل المحاضره مجانيه"} checked={lecture.isFree} onChange={(val) => changeStatus({ isFree: val })} isLoading={isLoading} />
            {canMakeCodes && (
              <div>
                <LinkMui href={"/management/codes?lecture=" + lecture._id + '&chapter=' + chapter._id} underline="hover" mr={'auto'} onClick={(e) => {
                  e.preventDefault()
      ...
      <CardActions sx={{ width: '100%' }} >
        <FlexBetween sx={{ width: '100%' }}>
          ...
          <FlexRow>
            ...
            {(lecture.sectionType === sectionConstants.VIDEO && showStatistics) && (
              <BtnModal
                fullScreen
                component={<GetViewsPage course={courseId} lecture={lecture._id} />} btn={<OutLinedHoverBtn endIcon={<FcStatistics />} colorm='orange'>
                  {lang.STATISTICS}
                </OutLinedHoverBtn>} />
              // <OutLinedHoverBtn
              //   colorm='orange'
