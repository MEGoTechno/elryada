...
function PaymentMethods({ coupon, price, handelResponse,
    ...
    const onSubmit = async (values) => {
        const res = await makeInvoice({
            ...
            name: invoiceNameId,
            description: subTitle + ' ' + '(السعر' + ' ' + price + 'ريال' + ')',
        }, true)
        ...
        <ModalStyled fullWidth={true} open={open} setOpen={setOpen}>
            <Box sx={{ width: '100%' }}>
                <Section>
                    ...
                    <InfoText label={'المبلغ المطلوب'} description={price + ' ريال'} />
                    ...
                    {payments?.length ? (
                        ...
                        <Alert sx={{my: '16px'}} variant="filled" severity="warning" >لا يوجد وسائل دفع متاحه حاليا !</Alert>
                    )}
                    <Box>
                        {chosenPayment && (
