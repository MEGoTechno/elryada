...
function GetCodesPage() {
    ...
    const fetchFc = async (params) => {
        ...
        params.populate = 'createdBy.name usedBy.name usedBy.userName usedBy.avatar'
    ...
    const updateFc = async (values) => {
        const res = await updateCode({ ...values, chapter })
        return res
    }
    ...
    const deleteFc = async (id) => {
        await deleteCode({ _id: id, chapter })
    }
    ...
    return (
        <Section>
            ...
            <ModalStyled open={open} setOpen={setOpen} >
                <CreateCode setReset={setReset} lecture={data?.values?.name ? data?.values : false} chapter={chapter} />
            </ModalStyled>
