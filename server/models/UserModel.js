...
const { user_roles, userRolesArr } = require("../tools/constants/rolesConstants")
...
const userSchema = new mongoose.Schema({
    ...
    government: { type: Number, enum: governments.map(g => Number(g.region_id)), default: governDefault },

    devicesAllowed: { type: Number, default: 3 }, // max 2
    devicesRegistered: [{ type: String, default: [] }],
    wallet: { type: Number, default: 0, max: [2000, "اقصى مبلغ هو 2000 ريال"] },
    fileConfirm: {
    ...
    role: {
        ...
        enum: userRolesArr.filter(role => role.isActive).map(role => role.id),
    },
    permissions: [{
        type: String
    }]
