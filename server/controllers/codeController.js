...
const getCodes = getAll(CodeModel, 'codes', codeParams, true)
...
const handelCreateCode = expressAsyncHandler(async (req, res, next) => {
    ...
    if (copies >= 1) {
        ...
        for (let i = 0; i < copies; i++) {
            ...
            const createdCode = await CodeModel.create({ ...code, code: codeName, createdBy: user._id })
...
const createCode = insertOne(CodeModel, null, null, { createdBy: true })
