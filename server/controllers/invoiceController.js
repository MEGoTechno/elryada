    ...
    switch (payment.type) {
        case paymentInteg.WALLET:
            if (invoice.price > user.wallet) {
                return next(createError('المحفظه لا تكفى, بالرجاء شحن مبلغ ' + (invoice.price - user.wallet) + ' ريال', 400, FAILED))
            }
        ...
        case paymentInteg.PAYMOB:
            ...
            const paymentToken = await generatePaymentKey(token, invoice.price * 100, orderId, {
                ...
                shipping_method: "NA",
                postal_code: "NA",
                city: governments.find(i => i.region_id === user.government)?.name_ar || 'المنصوره',
                country: "EG",
                state: governments.find(i => i.region_id === user.government)?.name_ar || 'المنصوره',
            });
