...
exports.handelOneFile = (fileKey) => //Previous upload without Storage
...
exports.insertOne = (Model, withIndex = false, populate = '', meta = {}) =>
    asyncHandler(async (req, res) => {

        if (withIndex) {
            const lastDoc = await Model.findOne().sort({ createdAt: -1 }).lean().select('index')
...
exports.makeLoginSession = () => {
    return asyncHandler(async (req, res, next) => {
        ...
        res.cookie('refreshToken', refreshToken, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',  // Secure only in production
            sameSite: process.env.sameSite === 'lax' ? 'lax' : 'none', // lax for local dev
        ...
        try {
            ...
            switch (code.type) {
                case codeConstants.WALLET:
                    ...
                    if ((user.wallet + code.price) >= 2000) return reject(createError("اقصى مبلغ للمحفظه 2000 ريال", 400, statusTexts.FAILED))
...
exports.pushToModel = (Model) => {
    return asyncHandler(async (req, res, next) => {
        ...
        const update = action === 'push' ? {
            ...
            $pull: {
                [field]: value //value should be single, if array, replaced by => {$in: value}
            }
        }
