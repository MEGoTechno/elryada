...
const { getAll, analysisMonthly, pushToModel, deleteMany, deleteOne, insertOne } = require("./factoryHandler.js");
...
const { defaultsPerms } = require("../tools/permissions/defaultsPerms.js");

const { chapterPerms } = require("../tools/permissions/courses.js");
...
const createUser = insertOne(UserModel)

const defaultsPermissionsTeacherAndSubadmin = expressAsyncHandler(async (req, res, next) => {
    const user = req.body
    if ([user_roles.SUBADMIN, user_roles.TEACHER].includes(user.role)) {
        req.body.permissions = defaultsPerms
    }
    return next()
})

const defaultsPermissionsOnChapter = expressAsyncHandler(async (req, res, next) => {
    const data = req.body

    if (data.field === 'chapters') {
        const teacher = data.id
        const chapterId = data.value

        const chapterPermissions = chapterPerms(chapterId, null, { values: true })
        // Create dynamic update object
        const update = data.action === 'push' ? {
            $addToSet: {
                permissions: chapterPermissions.flat().filter(ch => ch.isDefault).map(ch => ch.id)
            }
        } : {
            $pull: {
                permissions: { $in: chapterPermissions.flat().filter(ch => ch.scoped).map(ch => ch.id) }
            }
        }
        await UserModel.updateOne({ _id: teacher }, update)
    }
    return next()
})
// @desc get one user
...
// @route POST /users
// @access Private
const validateCreate = asyncHandler(async (req, res, next) => {
    ...
    if (user.role === user_roles.TEACHER) {
        const lastDoc = await UserModel.findOne().sort({ createdAt: -1 }).lean().select('index')
...
module.exports = {
    getUsers, getByUserName,
    defaultsPermissionsTeacherAndSubadmin, validateCreate, createUser,
    updateUserProfile, updateUser, deleteUser, userParams,
    addToUsers,
