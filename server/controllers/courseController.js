...
const afterCreation = async (req, createdObj) => {
    try {
        const user = req.user
        const courseDefaultPermissions = [
            ...coursesPerms(createdObj._id, null, { values: true }),
            ...chapterPerms(null, 'create', { courseId: createdObj._id })
        ].flat().filter(c => c.scoped && (c.isDefault ?? true)).map(c => c.id)

        const updatedUser = await UserModel.findByIdAndUpdate(user._id, {
            $addToSet: {
                courses: createdObj._id,
                permissions: courseDefaultPermissions
            }
        }, { new: true })
        console.log('permissions length ==>', updatedUser.permissions.length)
        return { user: updatedUser }
    } catch (error) {
        console.log('error from after Creation ==>', afterCreation)
    }
}

const removeCoursePermissions = expressAsyncHandler(async (req, res, next) => {
    const courseId = req.params.id
    const user = req.user
    const coPerms = coursesPerms(courseId, null, { values: true }).flat().filter(c => c.scoped).map(c => c.id)

    await UserModel.updateOne({ _id: user._id }, {
        $pull: {
            permissions: { $in: coPerms } // Correct way for multiple values
        }
    }, { new: true })
    return next()
})

const createCourse = insertOne(CourseModel, true, null, { createdBy: true, afterCreation })
const getCourses = getAll(CourseModel, 'courses', coursesParams, true) //user admin

const getCoursesValidation = expressAsyncHandler(async (req, res, next) => {
    const user = req.user
    if ([user_roles.SUBADMIN, user_roles.TEACHER].includes(user.role)) {
        const perms = coursesPerms(null, 'read')
        const canAccessAll = perms.some(p => (user.permissions || []).includes(p.id))
        if (!canAccessAll) {
            const userCourses = await UserModel.findById(user._id).select('courses').lean()
            req.query = { ...req.query, _id: userCourses.courses.length ? userCourses.courses : 'empty' }
        }
    }
    return next()
})
...
const getCourseLecturesAndCheckForUser = expressAsyncHandler(async (req, res, next) => {
    ...
    const [course, chapters] = await lockLectures(currentCourse, userCourse, user)
    return res.status(200).json({ status: SUCCESS, values: { course, chapters, currentIndex: userCourse?.currentIndex || 0 } })
})
...
const subscribe = expressAsyncHandler(async (req, res, next) => {
    ...
    if (currentCourse?.price > user.wallet) {
        return next(createError('المحفظه لا تكفى, بالرجاء شحن مبلغ ' + (currentCourse.price - user.wallet) + ' ريال', 400, FAILED))
    }
...
const lecturePassed = expressAsyncHandler(async (req, res, next) => {
    ...
    const chapterProgress = req.body.chapterProgress

    let user = req.user._id

    const userCourse = await UserCourseModel.findOne({ user, course: courseId }).lean();
    if (!userCourse) return next(createError('انت غير مشترك', 400, FAILED));

    const existingChapter = userCourse.chapters.find(
        ch => ch.chapter.toString() === chapterProgress.chapter.toString()
    );

    if (existingChapter) {
        await UserCourseModel.updateOne(
            { user, course: courseId, "chapters.chapter": chapterProgress.chapter },
            { $set: { "chapters.$.currentIndex": chapterProgress.currentIndex } }
        );
    } else {
        // Add new chapter
        await UserCourseModel.updateOne(
...
module.exports = {
    getCourses, getOneCourse, createCourse, updateCourse, checkDeleteCourse, deleteCourse, coursesParams,
    getCourseLecturesAndCheckForUser, getLectureAndCheck, lecturePassed, subscribe,
    getExam, createAttempt, linkCourse, getCoursesValidation, removeCoursePermissions
}