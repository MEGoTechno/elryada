...
//Lectures =>
// Admin => chapters
//      User => chapters + currentIndex

const getLectures = getAll(LectureModel, 'lectures', lectureParams, false, 'video') //used bu users
const insertLecture = insertOne(LectureModel, true, null, { createdBy: true })
...
const getLecturesForAdmin = expressAsyncHandler(async (req, res, next) => {
    ...
    const courseMatch = [...course.linkedTo, course._id]
    const [lectures, chapters] = await Promise.all([
        LectureModel.find(
            { course: courseMatch } //*_* $in make it array
        ).lean(),
        ChapterModel.find({ courses: { $in: courseMatch } }).lean()
    ])

    const handledChapters = chapters.map(ch => ({ ...ch, lectures: lectures.filter(lec => lec.chapters.includes(ch._id)) }))
    ...
    //Fc [{chapter, lectures}]
    res.json({ status: SUCCESS, values: { chapters: handledChapters } })
})
...
const protectGetLectures = expressAsyncHandler(async (req, res, next) => {
    ...
    if ([user_roles.ADMIN, user_roles.SUBADMIN, user_roles.TEACHER].includes(user.role)) {
        return next();
    }
